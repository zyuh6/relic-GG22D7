...
/**
 * Represents an elliptic curve point over an undecic extension over a prime
 * field.
 */
typedef struct {
	/** The first coordinate. */
...
/**
 * Pointer to an elliptic curve point over an undecic extension field.
 */
#if ALLOC == AUTO
typedef ep11_st ep11_t[1];
#else
typedef ep11_st *ep11_t;
#endif
 ...
 */
#if ALLOC == AUTO
#define ep11_null(A)				/* empty */
#else
#define ep11_null(A)				A = NULL
#endif
...
#if ALLOC == DYNAMIC
#define ep11_new(A)															\
	A = (ep11_t)calloc(1, sizeof(ep11_st));									\
	if (A == NULL) {														\
		THROW(ERR_NO_MEMORY);												\
	}																		\
	fp11_null((A)->x);														\
...
#elif ALLOC == AUTO
#define ep11_new(A)				/* empty */

#elif ALLOC == STACK
#define ep11_new(A)															\
	A = (ep11_t)alloca(sizeof(ep11_st));										\
	fp11_new((A)->x);														\
	fp11_new((A)->y);														\
	fp11_new((A)->z);														\
...
#elif ALLOC == AUTO
#define ep11_free(A)				/* empty */
#elif ALLOC == STACK
#define ep11_free(A)				A = NULL;
#endif

/**
 * Adds two points in an elliptic curve over an undecic extension field.
 * Computes R = P + Q.
 *
 ...
 */
#if EP_ADD == BASIC
#define ep11_add(R, P, Q)		ep11_add_basic(R, P, Q);
#elif EP_ADD == PROJC
#define ep11_add(R, P, Q)		ep11_add_projc(R, P, Q);
#endif

/**
 * Subtracts a point in an elliptic curve over an undecic extension field from
 ...
 */
#if EP_ADD == BASIC
#define ep11_sub(R, P, Q)		ep11_sub_basic(R, P, Q)
#elif EP_ADD == PROJC
#define ep11_sub(R, P, Q)		ep11_sub_projc(R, P, Q)
#endif

/**
 * Doubles a point in an elliptic curve over an undecic extension field.
 * Computes R = 2P.
 *
 * @param[out] R				- the result.
 ...
 */
#if EP_ADD == BASIC
#define ep11_dbl(R, P)			ep11_dbl_basic(R, P);
#elif EP_ADD == PROJC
#define ep11_dbl(R, P)			ep11_dbl_projc(R, P);
#endif

 /**
 * Multiplies a point in an elliptic curve over an undecic extension field.
 * Computes R = kP.
 *
 * @param[out] R			- the result.
 ...
 */
#if EP_MUL == BASIC
#define ep11_mul(R, P, K)		ep11_mul_basic(R, P, K)
#elif EP_MUL == SLIDE
#define ep11_mul(R, P, K)		ep11_mul_slide(R, P, K)
#elif EP_MUL == MONTY
#define ep11_mul(R, P, K)		ep11_mul_monty(R, P, K)
#elif EP_MUL == LWNAF || EP2_MUL == LWREG
#define ep11_mul(R, P, K)		ep11_mul_lwnaf(R, P, K)
#endif
...
/*============================================================================*/
/* Function prototypes for ep11                                                    */
/*============================================================================*/
/**
 * Initializes the elliptic curve over cubic extension.
 */
void ep11_curve_init(void);

/**
 * Finalizes the elliptic curve over cubic extension.
 */
void ep11_curve_clean(void);
 ...
 * @param[out] b			- the 'b' coefficient of the elliptic curve.
 */
void ep11_curve_get_b(fp11_t a);
 ...
 * @param[in] a				- the field element to multiply.
 */
void ep11_curve_mul_a(fp11_t c, const fp11_t a);
 ...
 * @param[in] a				- the field element to multiply.
 */
void ep11_curve_mul_b(fp11_t c, const fp11_t a);
...
/**
 * Returns the 2-order point of the group of points in the elliptic curve.
 *
 * @param[out] T2			- the returned 2-order point.
 */
void ep11_curve_get_T2x(fp_t T2);

/**
 * Returns (p+1)/2, where p is the characteristic of the prime of the elliptic curve.
 *
 * @param[out] h			- the returned value.
 */
 ...
 * @param[out] n			- the returned order.
 */
void ep11_curve_get_ord(bn_t n);
 ...
 * @param[out] h			- the returned cofactor.
 */
void ep11_curve_get_cof(bn_t h);
 ...
 * @param[in] h			- the cofactor of the group order.
 */
void ep11_curve_set(const fp11_t a, const fp11_t b, const ep11_t g, const bn_t r,
		const bn_t h);
 ...
 * @return the number of bytes.
 */
int ep11_size_bin(const ep11_t a, int pack);
 ...
 * @throw ERR_NO_BUFFER		- if the buffer capacity is invalid.
 */
void ep11_read_bin(ep11_t a, const uint8_t *bin, size_t len);
 ...
 * @throw ERR_NO_BUFFER		- if the buffer capacity is invalid.
 */
void ep11_write_bin(uint8_t *bin, size_t len, const ep11_t a, int pack);
 ...
 * @param[out] p			- the point to negate.
 */
void ep11_neg(ep11_t r, const ep11_t p);
 ...
 * @param[in] q				- the second point to add.
 */
void ep11_add_projc(ep11_t r, const ep11_t p, const ep11_t q);
 ...
 * @param[in] q				- the second point to add.
 */
void ep11_add_jacob(ep11_t r, const ep11_t p, const ep11_t q);
 ...
 * @param[in] p				- the point to double.
 */
void ep11_dbl_projc(ep11_t r, const ep11_t p);
 ...
 * @param[in] p				- the point to double.
 */
void ep11_dbl_jacob(ep11_t r, const ep11_t p);
 ...
 * @param[in] k				- the integer.
 */
void ep11_mul_slide(ep11_t r, const ep11_t p, const bn_t k);
 ...
 * @param[in] k				- the integer.
 */
void ep11_mul_monty(ep11_t r, const ep11_t p, const bn_t k);
 ...
 * @param[in] k				- the integer.
 */
void ep11_mul_lwnaf(ep11_t r, const ep11_t p, const bn_t k);
 ...
 * @param[in] k				- the integer.
 */
void ep11_mul_lwreg(ep11_t r, const ep11_t p, const bn_t k);
 ...
 * @param[in] k				- the integer.
 */
void ep11_mul_gen(ep11_t r, const bn_t k);
 ...
 * @param[in] P					- the point to multiply.
 */
void ep11_mul_cof(ep11_t r, const ep11_t p);
 ...
 * @param[in] p				- the point to multiply.
 */
void ep11_mul_pre_basic(ep11_t *t, const ep11_t p);
 ...
 * @param[in] p				- the point to multiply.
 */
void ep11_mul_pre_yaowi(ep11_t *t, const ep11_t p);
 ...
 * @param[in] p				- the point to multiply.
 */
void ep11_mul_pre_nafwi(ep11_t *t, const ep11_t p);
 ...
 * @param[in] p				- the point to multiply.
 */
void ep11_mul_pre_combs(ep11_t *t, const ep11_t p);
 ...
 * @param[in] p				- the point to multiply.
 */
void ep11_mul_pre_combd(ep11_t *t, const ep11_t p);
 ...
 * @param[in] p				- the point to multiply.
 */
void ep11_mul_pre_lwnaf(ep11_t *t, const ep11_t p);
 ...
 * @param[in] k				- the integer.
 */
void ep11_mul_fix_basic(ep11_t r, const ep11_t *t, const bn_t k);
 ...
 * @param[in] k				- the integer.
 */
void ep11_mul_fix_yaowi(ep11_t r, const ep11_t *t, const bn_t k);
 ...
 * @param[in] k				- the integer.
 */
void ep11_mul_fix_nafwi(ep11_t r, const ep11_t *t, const bn_t k);
 ...
 * @param[in] k				- the integer.
 */
void ep11_mul_fix_combs(ep11_t r, const ep11_t *t, const bn_t k);
 ...
 * @param[in] k				- the integer.
 */
void ep11_mul_fix_combd(ep11_t r, const ep11_t *t, const bn_t k);
 ...
 * @param[in] k				- the integer.
 */
void ep11_mul_fix_lwnaf(ep11_t r, const ep11_t *t, const bn_t k);
 ...
 * @param[in] m				- the second integer,
 */
void ep11_mul_sim_basic(ep11_t r, const ep11_t p, const bn_t k, const ep11_t q, const bn_t m);
 ...
 * @param[in] m				- the second integer,
 */
void ep11_mul_sim_trick(ep11_t r, const ep11_t p, const bn_t k, const ep11_t q, const bn_t m);
 ...
 * @param[in] m				- the second integer,
 */
void ep11_mul_sim_inter(ep11_t r, const ep11_t p, const bn_t k, const ep11_t q, const bn_t m);
 ...
 * @param[in] m				- the second integer,
 */
void ep11_mul_sim_joint(ep11_t r, const ep11_t p, const bn_t k, const ep11_t q, const bn_t m);
 ...
 * @param[in] n			- the number of elements to multiply.
 */
void ep11_mul_sim_lot(ep11_t r, const ep11_t p[], const bn_t k[], int n);
 ...
 * @param[in] m				- the second integer,
 */
void ep11_mul_sim_gen(ep11_t r, const bn_t k, const ep11_t q, const bn_t m);
 ...
 * @param[in] len			- the number of points to multiply.
 */
void ep11_mul_sim_dig(ep11_t r, const ep11_t p[], const dig_t k[], size_t len);
...
/**
 * Maps a byte array to a point in an elliptic curve over an undecic extension.
 *
 * @param[out] p			- the result.
 * @param[in] msg			- the byte array to map.
 * @param[in] len			- the array length in bytes.
 */
void ep11_map(ep11_t p, const uint8_t *msg, size_t len);

/**
 * Computes a power of the Gailbraith-Lin-Scott homomorphism of a point
 * represented in affine coordinates on a twisted elliptic curve over an
 * undecic exension. That is, Psi^i(P) = Twist(P)(Frob^i(unTwist(P)).
 * On the trace-zero group of an undecic twist, consists of a power of the
 * Frobenius map of a point represented in affine coordinates in an elliptic
 * curve over an undecic exension. Computes Frob^i(P) = (p^i)P.
 *
 * @param[out] r			- the result in affine coordinates.
 * @param[in] p				- a point in affine coordinates.
 * @param[in] i				- the power of the Frobenius map.
 */
void ep11_frb(ep11_t r, const ep11_t p, int i);

/**
 * Compresses a point in an elliptic curve over an undecic extension.
 *
 * @param[out] r			- the result.
 * @param[in] p				- the point to compress.
 */
void ep11_pck(ep11_t r, const ep11_t p);

/**
 * Decompresses a point in an elliptic curve over an undecic extension.
 *
 * @param[out] r			- the result.
 * @param[in] p				- the point to decompress.
 * @return if the decompression was successful
 */
int ep11_upk(ep11_t r, const ep11_t p);

/**
 * Adds two points in an elliptic curve over an undecic extension field.
 * Computes R = T + Q.
 * Returns some middle values for computing line functions. 
 *
 * @param[out] R				- the result.
 * @param[out] alpha			- a value for line functions.
 * @param[out] ZTQ				- a value for line functions.
 * @param[in] T					- the first point to add.
 * @param[in] Q					- the second point to add in affine coordinates.
 */
void ep11_shared_add(ep11_t R, fp11_t alpha, fp11_t ZTQ, const ep11_t T, const ep11_t Q);
 ...
 *
 * @param[out] R				- the result.
 * @param[out] YY				- a value for line functions.
 * @param[out] ZZ				- a value for line functions.
 * @param[out] beta				- a value for line functions.
 * @param[in] T					- the point to double.
 */
void ep11_shared_dbl(ep11_t R, fp11_t YY, fp11_t ZZ, fp11_t beta, const ep11_t T);

/**
 * Doubles a point in affine coordinates in an elliptic curve over an undecic extension field.
 * Computes R = 2T.
 * Returns some middle values for computing line functions.
 *
 * @param[out] R				- the result.
 * @param[out] YY				- a value for line functions.
 * @param[out] beta				- a value for line functions.
 * @param[in] T					- the point to double in affine coordinates, which leads to ZZ=1.
 */
void ep11_shared_dbl1(ep11_t R, fp11_t YY, fp11_t beta, const ep11_t T);
#endif /* !RLC_EPX_H */
