...
/**
 * Represents an undecic extension field element.
 */
typedef fp_t fp11_t[11];

/**
 * Represents a double-precision undecic extension field element.
 */
typedef dv_t dv11_t[11];
...
/**
 * Represents a 22-degree extension field element.
 *
 * This extension is constructed with the basis {1, v}, where v^2 = u is an
 * adjoined root in the underlying undecic extension.
 */
typedef fp11_t fp22_t[2];

/**
 * Represents a double-precision 22-degree extension field element.
 */
typedef dv11_t dv22_t[2];
...
/**
 * Initializes a double-precision undecic extension field element with a null
 * value.
 *
* @param[out] A			- the undecic extension element to initialize.
 */
#define dv11_null(A)															\
		dv_null(A[0]); dv_null(A[1]); dv_null(A[2]); dv_null(A[3]); dv_null(A[4]); dv_null(A[5]);dv_null(A[6]);dv_null(A[7]);dv_null(A[8]);dv_null(A[9]);dv_null(A[10]);							\

/**
 * Allocates a double-precision undecic extension field element.
 *
 * @param[out] A			- the new undecic extension field element.
 */
#define dv11_new(A)															\
		dv_new(A[0]); dv_new(A[1]);dv_new(A[2]);dv_new(A[3]); dv_new(A[4]);dv_new(A[5]);dv_new(A[6]);dv_new(A[7]);dv_new(A[8]);dv_new(A[9]);dv_new(A[10]);							\

/**
 * Frees a double-precision undecic extension field element.
 *
 * @param[out] A			- the undecic extension field element to free.
 */
#define dv11_free(A)															\
		dv_free(A[0]); dv_free(A[1]); dv_free(A[2]);dv_free(A[3]); dv_free(A[4]); dv_free(A[5]);dv_free(A[6]);dv_free(A[7]);dv_free(A[8]);dv_free(A[9]);dv_free(A[10]);					\

/**
 * Initializes an undecic extension field element with null.
 *
* @param[out] A			- the undecic extension element to initialize.
 */
#define fp11_null(A)															\
		fp_null(A[0]); fp_null(A[1]); fp_null(A[2]);fp_null(A[3]); fp_null(A[4]); fp_null(A[5]);fp_null(A[6]);fp_null(A[7]);fp_null(A[8]);fp_null(A[9]);fp_null(A[10]);						\

/**
 * Allocates an undecic extension field element.
 *
 * @param[out] A			- the new undecic extension field element.
 */
#define fp11_new(A)															\
		fp_new(A[0]); fp_new(A[1]);fp_new(A[2]);fp_new(A[3]); fp_new(A[4]);fp_new(A[5]);fp_new(A[6]);fp_new(A[7]);fp_new(A[8]);fp_new(A[9]);fp_new(A[10]);							\

/**
 * Frees an undecic extension field element.
 *
 * @param[out] A			- the undecic extension field element to free.
 */
#define fp11_free(A)															\
		fp_free(A[0]); fp_free(A[1]); fp_free(A[2]);fp_free(A[3]); fp_free(A[4]); fp_free(A[5]);fp_free(A[6]);fp_free(A[7]);fp_free(A[8]);fp_free(A[9]);fp_free(A[10]);							\

/**
 * Multiplies two undecic extension field elements. Computes
 * C = A * B.
 *
 * @param[out] C			- the result.
 * @param[in] A				- the first undecic extension field element.
 * @param[in] B				- the second undecic extension field element.
 */
#define fp11_mul(C, A, B)	fp11_mul_lazyr(C, A, B)

/**
 * Squares an undecic extension field elements. Computes
 * C = A * A.
 *
 * @param[out] C			- the result.
 * @param[in] A				- the undecic extension field element.
 */
#define fp11_sqr(C, A)	fp11_sqr_lazyr(C, A)
...
/**
 * Initializes a double-precision 22-degree  extension field element with a null
 * value.
 *
* @param[out] A			- the 22-degree  extension element to initialize.
 */
#define dv22_null(A)															\
		dv11_null(A[0]); dv11_null(A[1]); 							\

/**
 * Allocates a double-precision 22-degree extension field element.
 *
 * @param[out] A			- the new 22-degree extension field element.
 */
#define dv22_new(A)															\
		dv11_new(A[0]); dv11_new(A[1]);						\

/**
 * Frees a double-precision 22-degree extension field element.
 *
 * @param[out] A			- the 22-degree extension field element to free.
 */
#define dv22_free(A)															\
		dv11_free(A[0]); dv11_free(A[1]);						\

/**
 * Initializes a 22-degree extension field element with null.
 *
* @param[out] A			- the 22-degree extension element to initialize.
 */
#define fp22_null(A)															\
		fp11_null(A[0]); fp11_null(A[1]);						\

/**
 * Allocates a 22-degree extension field element.
 *
 * @param[out] A			- the new 22-degree extension field element.
 */
#define fp22_new(A)															\
		fp11_new(A[0]); fp11_new(A[1]);						\

/**
 * Frees a 22-degree extension field element.
 *
 * @param[out] A			- the 22-degree extension field element to free.
 */
#define fp22_free(A)															\
		fp11_free(A[0]); fp11_free(A[1]); 							\

/**
 * Multiplies two 22-degree extension field elements. Computes
 * C = A * B.
 *
 * @param[out] C			- the result.
 * @param[in] A				- the first 22-degree extension field element.
 * @param[in] B				- the second 22-degree extension field element.
 */
#define fp22_mul(C, A, B)	fp22_mul_lazyr(C, A, B)

/**
 * Squares a 22-degree extension field element. Computes
 * C = A * A.
 *
 * @param[out] C			- the result.
 * @param[in] A				- the 22-degree extension field element.
 */
#define fp22_sqr(C, A)	fp22_sqr_lazyr(C, A)
...
/**
 * Initializes the undecic extension field arithmetic module.
 */
void fp11_field_init(void);

/**
 * Copies the second argument to the first argument.
 *
 * @param[out] c			- the result.
 * @param[in] a				- the undecic extension field element to copy.
 */
void fp11_copy(fp11_t c, const fp11_t a);

/**
 * Returns the result of a comparison between two undecic extension field
 * elements.
 *
 * @param[in] a				- the first undecic extension field element.
 * @param[in] b				- the second undecic extension field element.
 * @return RLC_EQ if a == b, and RLC_NE otherwise.
 */
int fp11_cmp(const fp11_t a, const fp11_t b);

/**
 * Returns the result of a signed comparison between an undecic extension field
 * element and a digit.
 *
 * @param[in] a				- the undecic extension field element.
 * @param[in] b				- the digit.
 * @return RLC_EQ if a == b, and RLC_NE otherwise.
 */
int fp11_cmp_dig(const fp11_t a, const dig_t b);

/**
 * Assigns an undecic extension field element to a digit.
 *
 * @param[in] a				- the undecic extension field element.
 * @param[in] b				- the digit.
 */
void fp11_set_dig(fp11_t a, const dig_t b);

/**
 * Assigns zero to an undecic extension field element.
 *
 * @param[out] a			- the undecic extension field element to zero.
 */
void fp11_zero(fp11_t a);

/**
 * Tests if an undecic extension field element is zero or not.
 *
 * @param[in] a				- the undecic extension field element to test.
 * @return 1 if the argument is zero, 0 otherwise.
 */
int fp11_is_zero(const fp11_t a);

/**
 * Assigns a random value to an undecic extension field element.
 *
 * @param[out] a			- the undecic extension field element to assign.
 */
void fp11_rand(fp11_t a);

/**
 * Prints an undecic extension field element to standard output.
 *
 * @param[in] a				- the undecic extension field element to print.
 */
void fp11_print(const fp11_t a);

/**
 * Computes a power of an undecic extension field element.
 ...
 * @param[in] b				- the exponent.
 */
void fp11_exp(fp11_t c, const fp11_t a, const bn_t b);

/**
 * Computes a power of a 22-extension field element by a small exponent.
 * Faster formulas are used if the extension field element is cyclotomic.
 *
 * @param[out] c			- the result.
 * @param[in] a				- the basis.
 * @param[in] b				- the exponent.
 */
void fp11_exp_dig(fp11_t c, const fp11_t a, dig_t b);

/**
 * Extracts the square root of an undecic extension field element. Computes
 * c = sqrt(a). The other square root is the negation of c.
 *
 * @param[out] c			- the result.
 * @param[in] a				- the extension field element.
 * @return					- 1 if there is a square root, 0 otherwise.
 */
int fp11_srt(fp11_t c, const fp11_t a);

/**
 * Tests if an undecic extension field element is a quadratic residue.
 *
 * @param[in] a				- the prime field element to test.
 * @return 1 if the argument is even, 0 otherwise.
 */
int fp11_is_sqr(const fp11_t a);

/**
 * Adds two undecic extension field elements. Computes c = a + b.
 *
 * @param[out] c			- the result.
 * @param[in] a				- the first undecic extension field element.
 * @param[in] b				- the second undecic extension field element.
 */
void fp11_add(fp11_t c,const fp11_t a,const fp11_t b);

/**
 * Double two undecic extension field elements. Computes c = 2a.
 *
 * @param[out] c			- the result.
 * @param[in] a				- the undecic extension field element.
 */
void fp11_dbl(fp11_t c,const fp11_t a);

/**
 * Substract two undecic extension field elements. Computes c = a-b.
 *
 * @param[out] c			- the result.
 * @param[in] a				- the first undecic extension field element.
 * @param[in] b				- the second undecic extension field element.
 */
void fp11_sub(fp11_t c, const fp11_t a, const fp11_t b);

/**
 * half an undecic extension field element. Computes c = a/2.
 *
 * @param[out] c			- the result.
 * @param[in] a				- the  undecic extension field element.
 */
void fp11_hlv(fp11_t c, const fp11_t a);

/**
 * Negates an undecic extension field element.
 */
void fp11_neg(fp11_t c, const fp11_t a);

/**
 * Reduces an undecic extension element modulo the configured prime p.
 * Computes c = a mod p.
 *
 * @param[out] c			- the result.
...
void fp11_rdc(fp11_t c, dv11_t a);

/**
 * Multiples two undecic extension field elements using lazy reduction.
 *
 * @param[out] c			- the result.
 * @param[in] a				- the 13-th extension field element.
 * @param[in] b				- the 13-th extension field element.
 */
void fp11_mul_lazyr(fp11_t d, const fp11_t a, const fp11_t b);

/**
 * Multiplies an undecic extension field element by the adjoined root.
 * Computes c = a * u.
 *
 * @param[out] c			- the result.
 * @param[in] a				- the undecic extension field element to multiply.
 */
void fp11_mul_art(fp11_t c, const fp11_t a);

/**
 * Multiplies an undecic extension field element by the inverse of the adjoined root.
 * Computes c = a * (1/u).
 *
 * @param[out] c			- the result.
 * @param[in] a				- the undecic extension field element to multiply.
...
/**
 * Squaring an undecic extension field element using lazy reduction.
 *
 * @param[out] c			- the result.
 * @param[in] a				- the 13-th extension field element.
 */
...
/**
 * Computes a power of the Frobenius map of an undecic extension field element.
 *
 * @param[out] c			- the result.
 * @param[in] a				- the undecic extension element to conjugate.
 * @param[in] i				- the power of the Frobenius map.
 */
void fp11_frb(fp11_t c, const fp11_t a, int i);

/**
 * Computes a power of the Frobenius map of the image of a 22-degree extension field element.
 *
 * @param[out] c			- the result.
 * @param[in] a				- the undecic extension element to conjugate.
 * @param[in] i				- the power of the Frobenius map.
 */
void fp11_frb2(fp11_t c, const fp11_t a, int i);

/**
 * Inverts an undecic extension field element. Computes c = 1/a.
 *
 * @param[out] c			- the result.
 * @param[in] a				- the undecic extension field element to invert.
 */
void fp11_inv(fp11_t c, const fp11_t a);

/**
 * Inverts undecic octic extension field elements simultaneously.
 *
 * @param[out] c			- the result.
 * @param[in] a				- the undecic extension field elements to invert.
 * @param[in] n				- the number of elements.
 */
void fp11_inv_sim(fp11_t * c, const fp11_t * a, int n);
...
/**
 * Initializes the 22-degree extension field arithmetic module.
 */
void fp22_field_init(void);

/**
 * Copies the second argument to the first argument.
 *
 * @param[out] c			- the result.
 * @param[in] a				- the 22-degree extension field element to copy.
 */
void fp22_copy(fp22_t c, const fp22_t a);

/**
 * Returns the result of a comparison between two 22-degree extension field
 * elements.
 *
 * @param[in] a				- the first 22-degree extension field element.
 * @param[in] b				- the second 22-degree extension field element.
 * @return RLC_EQ if a == b, and RLC_NE otherwise.
 */
int fp22_cmp(const fp22_t a, const fp22_t b);

/**
 * Returns the result of a signed comparison between a 22-degree extension field
 * element and a digit.
 *
 * @param[in] a				- the 22-degree extension field element.
 * @param[in] b				- the digit.
 * @return RLC_EQ if a == b, and RLC_NE otherwise.
 */
int fp22_cmp_dig(const fp22_t a, const dig_t b);

/**
 * Assigns a 22-degree extension field element to a digit.
 *
 * @param[in] a				- the 22-degree extension field element.
 * @param[in] b				- the digit.
 */
void fp22_set_dig(fp22_t a, const dig_t b);

/**
 * Assigns zero to a 22-degree extension field element.
 *
 * @param[out] a			- the 22-degree extension field element to zero.
 */
void fp22_zero(fp22_t a);

/**
 * Tests if a 22-degree extension field element is zero or not.
 *
 * @param[in] a				- the 22-degree extension field element to test.
 * @return 1 if the argument is zero, 0 otherwise.
 */
int fp22_is_zero(const fp22_t a);

/**
 * Assigns a random value to a 22-degree extension field element.
 *
 * @param[out] a			- the 22-degree extension field element to assign.
 */
void fp22_rand(fp22_t a);

/**
 * Prints a 22-degree extension field element to standard output.
 *
 * @param[in] a				- the 22-degree extension field element to print.
 */
void fp22_print(const fp22_t a);

/**
 * Computes a power of a 22-degree extension field element.
 * Faster formulas are used if the extension field element is cyclotomic.
 *
 * @param[out] c			- the result.
 * @param[in] a				- the basis.
 * @param[in] b				- the exponent.
 */
void fp22_exp(fp22_t c, const fp22_t a, const bn_t b);
 ...
 * @param[in] b				- the exponent.
 */
void fp22_exp_dig(fp22_t c, const fp22_t a, dig_t b);

/**
 * Extracts the square root of a 22-degree extension field element. Computes
 * c = sqrt(a). The other square root is the negation of c.
 *
 * @param[out] c			- the result.
 * @param[in] a				- the extension field element.
 * @return					- 1 if there is a square root, 0 otherwise.
 */
int fp22_srt(fp22_t c, const fp22_t a);

/**
 * Tests if a 22-degree extension field element is a quadratic residue.
 *
 * @param[in] a				- the prime field element to test.
 * @return 1 if the argument is even, 0 otherwise.
 */
int fp22_is_sqr(const fp22_t a);

/**
 * Adds two 22-degree extension field elements. Computes c = a + b.
 *
 * @param[out] c			- the result.
 * @param[in] a				- the first 22-degree extension field element.
 * @param[in] b				- the second 22-degree extension field element.
 */
void fp22_add(fp22_t c,const fp22_t a,const fp22_t b);

/**
 * Double two 22-degree extension field elements. Computes c = 2a.
 *
 * @param[out] c			- the result.
 * @param[in] a				- the 22-degree extension field element.
 */
void fp22_dbl(fp22_t c,const fp22_t a);

/**
 * Substract two 22-degree extension field elements. Computes c = a-b.
 *
 * @param[out] c			- the result.
 * @param[in] a				- the first 22-degree extension field element.
 * @param[in] b				- the second 22-degree extension field element.
 */
void fp22_sub(fp22_t c, const fp22_t a, const fp22_t b);

/**
 * half a 22-degree extension field element. Computes c = a/2.
 *
 * @param[out] c			- the result.
 * @param[in] a				- the  22-degree extension field element.
 */
void fp22_hlv(fp22_t c, const fp22_t a);

/**
 * Negates a 22-degree extension field element.
 */
void fp22_neg(fp22_t c, const fp22_t a);

/**
 * Reduces a 22-degree extension element modulo the configured prime p.
 * Computes c = a mod p.
 *
 * @param[out] c			- the result.
 * @param[in] a				- the digit vector to reduce.
...
void fp22_rdc(fp22_t c, dv22_t a);

/**
 * Multiples two 22-degree extension field elements using lazy reduction.
 *
 * @param[out] c			- the result.
 * @param[in] a				- the 13-th extension field element.
 * @param[in] b				- the 13-th extension field element.
...
/**
 * Squaring a 22-degree extension field element using lazy reduction.
 *
 * @param[out] c			- the result.
 * @param[in] a				- the 13-th extension field element.
 */
void fp22_sqr_lazyr(fp22_t d, const fp22_t a);

/**
 * Computes a power of the Frobenius map of a 22-degree extension field element.
 *
 * @param[out] c			- the result.
 * @param[in] a				- the 22-degree extension element to conjugate.
 * @param[in] i				- the power of the Frobenius map.
 */
void fp22_frb(fp22_t c, const fp22_t a, int i);

/**
 * Inverts a 22-degree extension field element. Computes c = 1/a.
 *
 * @param[out] c			- the result.
 * @param[in] a				- the 22-degree extension field element to invert.
 */
void fp22_inv(fp22_t c, const fp22_t a);

/**
 * Inverts 22-degree octic extension field elements simultaneously.
 *
 * @param[out] c			- the result.
 * @param[in] a				- the 22-degree extension field elements to invert.
 * @param[in] n				- the number of elements.
 */
void fp22_inv_sim(fp22_t * c, const fp22_t * a, int n);

/**
 * Computes the inverse of a cyclotomic 22-degree extension field element.
 *
 * For cyclotomic elements, this is equivalent to computing the conjugate.
 * A cyclotomic element is one previously raised to the (p^11 - 1)-th power.
 *
 * @param[out] c                        - the result.
 * @param[in] a                         - the 22-degree extension field element to invert.
 */
void fp22_inv_cyc(fp22_t c, const fp22_t a);
...
void fp11_addc_low(dv11_t c, dv11_t a, dv11_t b);
void fp11_addd_low(dv11_t c, dv11_t a, dv11_t b);
void fp11_subc_low(dv11_t c, dv11_t a, dv11_t b);
void fp11_addn_low(fp11_t c, const fp11_t a, const fp11_t b);
void fp11_muln_low(dv11_t c, const fp11_t a, const fp11_t b);
void fp11_sqrn_low(dv11_t c, const fp11_t a);

/**
 * Multiplies an undecic extension field by the quadratic non-residue.
 * Computes C = A * E, where E is a non-square in the quadratic
 * extension field.
 * c = a * u (Fp11=Fp(u))
 *
 * @param[out] C			- the result.
...
/**
 * Computes the square of a cyclotomic 22-degree extension field element.
 *
 * @param[out] c			- the result.
 * @param[in] a				- the cyclotomic extension element to square.
 */
...
/**
 * Computes the x-th power of a cyclotomic 22-degree extension field element 
 * using addition chain.
 * x=-779523 is the seed. 
 *
 * @param[out] c			- the result.
 * @param[in] a				- the basis.
 */
void fp22_exp_cyc_x(fp22_t c, const fp22_t a);
