...
{
	return (fp_cmp(a[0], b[0]) == RLC_EQ) && (fp_cmp(a[1], b[1]) == RLC_EQ) && (fp_cmp(a[2], b[2]) == RLC_EQ) && (fp_cmp(a[3], b[3]) == RLC_EQ)
		&& (fp_cmp(a[4], b[4]) == RLC_EQ) && (fp_cmp(a[5], b[5]) == RLC_EQ) && (fp_cmp(a[6], b[6]) == RLC_EQ) && (fp_cmp(a[7], b[7]) == RLC_EQ)
		&& (fp_cmp(a[8], b[8]) == RLC_EQ) && (fp_cmp(a[9], b[9]) == RLC_EQ) && (fp_cmp(a[10], b[10]) == RLC_EQ) ? RLC_EQ : RLC_NE;
}

int fp11_cmp_dig(const fp11_t a, dig_t b) {
	return (fp_cmp_dig(a[0], b) == RLC_EQ) && fp_is_zero(a[1]) && fp_is_zero(a[2]) && fp_is_zero(a[3]) && fp_is_zero(a[4])
		&& fp_is_zero(a[5]) && fp_is_zero(a[6]) && fp_is_zero(a[7]) && fp_is_zero(a[8]) && fp_is_zero(a[9]) && fp_is_zero(a[10]) ?
			RLC_EQ : RLC_NE;
}
...
int fp22_cmp(const fp22_t a, const fp22_t b) {
	return (fp11_cmp(a[0], b[0]) == RLC_EQ) && (fp11_cmp(a[1], b[1]) == RLC_EQ) ? RLC_EQ : RLC_NE;
}

int fp22_cmp_dig(const fp22_t a, const dig_t b) {
	return (fp11_cmp_dig(a[0], b) == RLC_EQ) && fp11_is_zero(a[1]) ? RLC_EQ : RLC_NE;
}
